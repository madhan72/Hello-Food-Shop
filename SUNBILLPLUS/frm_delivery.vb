...
Public Class frm_delivery
    ...
    Dim da, da_head, da_detl, da_party, da_currency, da_godown As SqlDataAdapter
    Dim bs As New BindingSource
    Dim ds, ds_location, ds_autotype, ds_party, ds_currency, ds1, ds_head, ds_detl, ds_item, ds_acdesc, ds_goodwn As New DataSet
    Dim Showpartyfindform As Boolean
    Dim dt As New DataTable
    Dim dr As SqlDataReader
    Dim Locationid_t, Itemid_t, Hsnid_t, Partyid_t, Currencyid_v As Double, Formshown_t, Isalreadyexistflag_t As Boolean, SavechhkFlg As Boolean, Addlessformulaflg_t As Boolean
    Dim Controlno_t As String, Process As String, Trntype_t As String, Sqlstr As String, Colname_t As String, HsnCode_t As String
    Dim Headerid_t As Double, AcDescid_t As Double, Cardid_t As Double, godownid_t As Double, Dcid_t As Double
    ...
    Dim PROCESSTYPE As String, PREFIX As String, SUFFIX As String
    Dim ds_settings As New DataSet
    Dim AutoTypeId_t As Double
    ...
    Private Sub dsopen()
        Try
            ...
            Sqlstr = "SELECT isnull(A.Prefix, '') + REPLICATE('0', ISNULL(A.Noofdigit,0) - len(A.SEQNO)) + cast(A.seqno as varchar(30)) + isnull(A.Suffix,'') as seqno, " _
            & "Isnull(A.Autonoid,0) As Autotypeid,Isnull(A.Prefix,'') As Prefix,Isnull(A.Suffix,'') As Suffix,A.Processtype, " _
            & "Isnull(A.Billcompid,0) As Billcompid,C.Compname As Billcompany,Isnull(A.Taxflag,0) As Taxflag FROM Autonumber A " _
            & "Left Join Company C On C.Compid = A.Billcompid WHERE A.process = '" & Process & "' And A.Compid = " & Gencompid & "  "
            cmd = New SqlCommand(Sqlstr, conn)
            cmd.CommandType = CommandType.Text
            da = New SqlDataAdapter(cmd)
            ds_autotype = New DataSet
            ds_autotype.Clear()
            da.Fill(ds_autotype)
    ...
    Private Sub clearchars()
        Try
            ...
            If editflag = False Then
                If ds_autotype.Tables(0).Rows.Count = 1 Then
                    PROCESSTYPE = ds_autotype.Tables(0).Rows(0).Item("Processtype").ToString
                    PREFIX = ds_autotype.Tables(0).Rows(0).Item("Prefix").ToString
                    SUFFIX = ds_autotype.Tables(0).Rows(0).Item("Suffix").ToString

                    Vatcstflag_t = ds_autotype.Tables(0).Rows(0).Item("Taxflag").ToString
                    txt_vchnum.Text = AutoNum_type(Process, PROCESSTYPE, PREFIX, False, SUFFIX)
                    txt_vchnum.Enabled = False
                    txt_vchnum.BackColor = Color.White
                ElseIf ds_autotype.Tables(0).Rows.Count = 0 Then
                    txt_vchnum.Enabled = False
                Else
                    txt_vchnum.Enabled = True
                    PROCESSTYPE = ""
                    PREFIX = ""
                    SUFFIX = ""
                End If
    ...
    Private Sub Headercall(ByVal Headerid_v As Double)
        Try
            cmd = New SqlCommand("SELECT H.VCHNUM,H.HEADERID,H.VCHDATE,H.TRNTYPE,ISNULL( H.DCID,0) AS DCID,ISNULL(DH.VCHNUM,'') AS DCNO   " _
                                 & "  ,ISNULL(H.CHEFID,0) AS CHEFID, p.PTYNAME as CHEF,ISNULL(H.NARRATION,'') AS NARRATION,ISNULL(GM.GODOWNNAME,'') AS LOCATION,ISNULL(H.LOCATIONID,0) AS LOCATIONID  FROM DELIVERY_HEADER H " _
                                 & " LEFT JOIN PARTY P ON P.PTYCODE = H.CHEFID LEFT JOIN GODOWN_MASTER GM ON GM.MASTERID = H.LOCATIONID  LEFT JOIN DELIVERY_HEADER DH ON DH.HEADERID =H.DCID WHERE H.HEADERID = " & Headerid_v & " ", conn)
            cmd.CommandType = CommandType.Text
    ...
    Private Sub storechars(Optional ByVal pos As Integer = 0)
        Try
            ...
            godownid_t = ds_head.Tables(0).Rows(rowid_t).Item("LOCATIONID")
            ...
            txt_supplier.Text = ds_head.Tables(0).Rows(rowid_t).Item("CHEF").ToString
            '   txt_reference.Text = ds_head.Tables(0).Rows(rowid_t).Item("reference").ToString
            Partyid_t = ds_head.Tables(0).Rows(rowid_t).Item("CHEFID")

            '   Txt_currency.Text = ds_head.Tables(0).Rows(rowid_t).Item("CURRENCY").ToString
            ...
            cmd.CommandText = "get_deliverydetail"
            ...
            If Detlcnt_t > 0 Then
                ...
                For i = 0 To Detlcnt_t - 1
                    ...
                    ' GridView1.Rows(i).Cells(fields1.c1_costrate).Value = ds_detl.Tables(0).Rows(i).Item("Rate")
                    ...
                    GridView1.Rows(i).Cells(fields1.c1_itemdesc).Value = ds_detl.Tables(0).Rows(i).Item("ITEMDES")
    ...
    Private Sub calcnetamt()
        Try
            ...
            txt_netamt.Text = Format((Val(txt_totamount.Text)), "#######0")
        ...
        Try
            ...
            Select Case colindex_t
                Case fields1.c1_itemcode
                    ...
                    If Val_t = 0 Then
                        Sqlstr = "SELECT IM.ITEMCODE,IM.ITEMID,IM.ITEMDES as ITEMTAMILDES,IM.ITEMTAMILDES as ITEMDES,ISNULL(HM.HSNCODE,'') AS HSNCODE,isnull(im.SELPRICERETAIL,0) as SELPRICERETAIL, ISNULL(IM.COSTPRICE,0) AS COSTRATE,UM.UOM,isnull(um.noofdecimal,0) as noofdecimal ,ISNULL(IM.UOMID,0) AS UOMID,ISNULL(IM.HSNID,0) AS HSNID FROM ITEM_MASTER IM " _
                        & " LEFT JOIN UOM_MASTER UM ON UM.MASTERID = IM.UOMID LEFT JOIN HSNACCOUNTCODE_MASTER HM ON HM.MASTERID = IM.HSNID where im.itemid not in (" & Nextcond_t & ") AND SIGN(im.itemid) =1 AND IM.ITEMTYPE='RAW MATERIAL' ORDER BY IM.ITEMCODE "
                    Else
                        Sqlstr = "SELECT IM.ITEMCODE,IM.ITEMID,IM.ITEMDES,IM.ITEMTAMILDES,isnull(im.SELPRICERETAIL,0) as SELPRICERETAIL,ISNULL(HM.HSNCODE,'') AS HSNCODE,ISNULL(IM.COSTPRICE,0) AS COSTRATE,UM.TAMILUOM AS UOM,isnull(um.noofdecimal,0) as noofdecimal ,ISNULL(IM.UOMID,0) AS UOMID,ISNULL(IM.HSNID,0) AS HSNID FROM ITEM_MASTER IM " _
                        & " LEFT JOIN UOM_MASTER UM ON UM.MASTERID = IM.UOMID LEFT JOIN HSNACCOUNTCODE_MASTER HM ON HM.MASTERID = IM.HSNID  where im.itemid not in (" & Nextcond_t & ")  AND SIGN(im.itemid) =1 AND IM.ITEMTYPE='RAW MATERIAL' ORDER BY IM.ITEMCODE "
                    End If
                    ...
                    If ds_item.Tables(0).Rows.Count > 0 Then
                        ...
                        If ds_item.Tables(0).Rows.Count = 1 Then
                            ...
                            Uomid_t = ds_item.Tables(0).Rows(0).Item("uomid")
                        ...
                        Else
                            ...
                            If Val_t = 0 Then
                                Sqlstr = "SELECT IM.ITEMCODE,IM.ITEMID,IM.ITEMDES as ITEMTAMILDES, ISNULL(HM.HSNCODE,'') AS HSNCODE,IM.ITEMTAMILDES as ITEMDES,isnull(im.SELPRICERETAIL,0) as SELPRICERETAIL,ISNULL(IM.COSTPRICE,0) AS COSTRATE,UM.UOM,IM.UOMID,ISNULL(IM.TAXPERC,0) AS TAXPERC,isnull(um.NOOFDECIMAL,0) as NOOFDECIMAL,ISNULL(IM.HSNID,0) AS HSNID FROM ITEM_MASTER IM " _
                      & " LEFT JOIN UOM_MASTER UM ON UM.MASTERID = IM.UOMID LEFT JOIN HSNACCOUNTCODE_MASTER HM ON HM.MASTERID = IM.HSNID  WHERE SIGN(im.itemid) =1 and im.itemcode ='" & tmppassstr & "' AND IM.ITEMTYPE='RAW MATERIAL'  ORDER BY IM.ITEMCODE "
                            Else
                                Sqlstr = "SELECT IM.ITEMCODE,IM.ITEMID,IM.ITEMDES,IM.ITEMTAMILDES,ISNULL(HM.HSNCODE,'') AS HSNCODE,isnull(im.SELPRICERETAIL,0) as SELPRICERETAIL ,ISNULL(IM.COSTPRICE,0) AS COSTRATE,UM.TAMILUOM AS UOM, IM.UOMID,ISNULL(IM.TAXPERC,0) AS TAXPERC,isnull(um.NOOFDECIMAL,0) as NOOFDECIMAL,ISNULL(IM.HSNID,0) AS HSNID FROM ITEM_MASTER IM " _
                      & " LEFT JOIN UOM_MASTER UM ON UM.MASTERID = IM.UOMID  LEFT JOIN HSNACCOUNTCODE_MASTER HM ON HM.MASTERID = IM.HSNID  WHERE  SIGN(im.itemid) =1 and im.itemcode ='" & tmppassstr & "' AND IM.ITEMTYPE='RAW MATERIAL'  ORDER BY IM.ITEMCODE "
                            End If
                        ...
                        If ds_item.Tables(0).Rows.Count > 0 And fm.VarNewid <> 0 Then
                            ...
                            For i = 0 To ds_item.Tables(0).DefaultView.Count - 1
                                ...
                                Uomid_t = ds_item.Tables(0).Rows(i + index_t).Item("uomid")
                                ...
                                If Tmpitemid_t <> 0 Then
                                    ...
                                    GridView1.Rows(activerow + i).Cells(fields1.c1_uomid).Value = Uomid_t
    ...
    Private Sub Saveproc(ByVal editflag_t As Boolean)
        Try
            Dim i As Integer
            Dim Rate_t As Double, amount_t As Double, uomid_t, Qty_t As Double, remarks_v As String
            Dim Adtype_t As String, Adamt_t As Double, Adperc_t As Double, Addescid_t As Double, CostRate_t As Double

            If editflag = False Then
                txt_vchnum.Text = AutoNum(Process, True) ' AutoNum(Process, PROCESSTYPE, PREFIX, True, SUFFIX) 'if
            Else
                trans = conn.BeginTransaction(IsolationLevel.ReadCommitted)
            End If
            SavechhkFlg = False
            Headerid_t = GensaveDEliveryHead(IIf(editflag, 1, 0), Headerid_t, Trim(txt_vchnum.Text), DTP_Vchdate.Value, Partyid_t,
                                                  Gencompid, godownid_t, Val(txt_netamt.Text), Trim(txt_narration.Text), 0, Trntype_t)
            ...
            For i = 0 To GridView1.Rows.Count - 1
                ...
                If Itemid_t <> 0 And Qty_t <> 0 Then
                    If remarks_v = "" Or remarks_v Is Nothing Then remarks_v = "  "
                    GensaveDeliverydetl(Headerid_t, txt_vchnum.Text, DTP_Vchdate.Value, godownid_t, Rate_t, amount_t, Gencompid, Itemid_t, Qty_t, remarks_v, uomid_t, Hsnid_t)
                    SavechhkFlg = True
                End If
            Next
        ...
        Try
            ...
            If e.RowIndex >= 0 And e.ColumnIndex >= 0 Then
                ...
                If GridView1.Rows.Count - 1 = e.RowIndex Then
                    If IsValidRow(GridView1, "C_Itemcode", "C_Itemid") Then
    ...
    Private Sub cmd_ok_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmd_ok.Click
        Try
            ...
            ElseIf Partyid_t = 0 Or Trim(txt_supplier.Text) = "" Then
                MsgBox("Chef Name Should not be Empty.")
                txt_supplier.Focus()
            Else
    ...
    Public Sub Deleteproc(ByVal Headerid_v As Double)
        ...
        Try
            If MsgBox("Are you sure ?", MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton2, "Delete") = MsgBoxResult.Yes Then
                Call GendelDelivery(Headerid_v)
                Exit Sub
            End If
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
    ...
    Public Sub Print_Rpt(ByVal Headerid_v As Double)
        Try
            ...
            rm.Init(conn, "DELIVERY", Servername_t, Headerid_v, Nothing, Nothing, "", conn.Database, 0, False, , , , , , SystemName_t)
    ...
    Private Sub VchnumFindfrm()
        Try
            VisibleCols.Add("Seqno")
            Colheads.Add("No")
            ...
            fm.MainForm = New Frm_purchaseorder
            fm.Active_ctlname = "txt_Vchnum"
            Csize.Add(200)

            tmppassstr = txt_vchnum.Text
            fm.EXECUTE(conn, ds_autotype, VisibleCols, Colheads, AutoTypeId_t, "", False, Csize, "", False, False, "", tmppassstr)
            txt_vchnum.Text = fm.VarNew
            AutoTypeId_t = fm.VarNewid

            If ds_autotype.Tables(0).Rows.Count > 0 And AutoTypeId_t <> 0 Then
                ds_autotype.Tables(0).DefaultView.RowFilter = "Autotypeid = " & AutoTypeId_t & " "
                index_t = ds_autotype.Tables(0).Rows.IndexOf(ds_autotype.Tables(0).DefaultView.Item(0).Row)
                txt_vchnum.Text = ds_autotype.Tables(0).Rows(index_t).Item("Seqno").ToString

                PROCESSTYPE = ds_autotype.Tables(0).Rows(index_t).Item("Processtype").ToString
                PREFIX = ds_autotype.Tables(0).Rows(index_t).Item("Prefix").ToString
                SUFFIX = ds_autotype.Tables(0).Rows(index_t).Item("Suffix").ToString
            End If
    ...
    Private Sub Partyfindfrm()
        Try
            ...
            ds_party.Clear()
            ds_party = Nothing
            Sqlstr = "Select Ptyname,ptycode From PARTY WHERE PTYTYPE ='CHEF' Order By Ptyname"
            cmd = New SqlCommand(Sqlstr, conn)
            cmd.CommandType = CommandType.Text
    ...
    Private Sub Locationfindfrm()
        Try
            ...
            cmd = New SqlCommand(Sqlstr, conn)
            cmd.CommandType = CommandType.Text
            da_godown = New SqlDataAdapter(cmd)
            ds_goodwn = New DataSet
            ds_goodwn.Clear()
            da_godown.Fill(ds_goodwn)
            cnt = ds_goodwn.Tables(0).Rows.Count
            ...
            If cnt > 0 Then
                ...
                If cnt = 1 Then
                    txt_location.Text = ds_goodwn.Tables(0).Rows(0).Item("godownname").ToString
                    godownid_t = ds_goodwn.Tables(0).Rows(0).Item("masterid")
                Else
                    tmppassstr = txt_location.Text
                    fm.EXECUTE(conn, ds_goodwn, VisibleCols, Colheads, godownid_t, "", False, Csize, "", False, False, "", tmppassstr)
                    txt_location.Text = fm.VarNew
                    godownid_t = fm.VarNewid
                End If
    ...
    Private Sub Frm_purchaseorder_Load(sender As Object, e As EventArgs) Handles Me.Load
        Try
    ...
    Private Sub Execute()
        Try
            ...
            Call gridreadonly(GridView1, True, "C_Itemdes", "c_uom", "c_uomid", "C_Amount", "c_hsncode")
            Call gridreadonly_Color(GridView1, Readonlycolor_t, "C_Itemdes", "c_uom", "c_uomid", "C_Amount", "c_hsncode")
            Call gridvisible(GridView1, False, "C_Itemid", "c_uomid", "c_decimal", "c_Rate", "C_Amount", "c_costrate")
    ...
    Private Sub GridView1_RowPostPaint(sender As Object, e As DataGridViewRowPostPaintEventArgs) Handles GridView1.RowPostPaint
        Using b As SolidBrush = New SolidBrush(GridView1.RowHeadersDefaultCellStyle.ForeColor)
            e.Graphics.DrawString(e.RowIndex + 1.ToString(System.Globalization.CultureInfo.CurrentUICulture), _
                                   IIf(font1 Is Nothing, DefaultFont, font1), _
                                   b, _
    ...
    Private Sub txt_vchnum_Click(sender As Object, e As EventArgs) Handles txt_vchnum.Click
        Call VchnumFindfrm()
    End Sub

    Private Sub txt_vchnum_GotFocus(sender As Object, e As EventArgs) Handles txt_vchnum.GotFocus
        txt_vchnum.BackColor = Color.Yellow
    End Sub

    Private Sub txt_vchnum_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_vchnum.KeyDown
    ...
    Private Function Isalreadyexists() As Boolean
        Try
            Dim tmpds As New DataSet
            Sqlstr = "Select H.Headerid,H.Vchnum From delivery_Header H " _
                   & "Where H.Vchnum = '" & txt_vchnum.Text & "' "
            cmd = New SqlCommand(Sqlstr, conn)
            cmd.CommandType = CommandType.Text
            da = New SqlDataAdapter(cmd)
            tmpds = New DataSet
            ...
            If tmpds.Tables(0).Rows.Count > 0 Then
                ...
                Headerid_t = tmpds.Tables(0).Rows(0).Item("Headerid")
            Else
    ...
    Private Sub txt_vchnum_LostFocus(sender As Object, e As EventArgs) Handles txt_vchnum.LostFocus
        txt_vchnum.BackColor = Color.White
        Isalreadyexistflag_t = Isalreadyexists()
        If Isalreadyexistflag_t = True Then
            Call Headercall(Headerid_t)
    ...
    Private Sub txt_deliveryto_Click(sender As Object, e As EventArgs) Handles txt_location.Click
        Locationfindfrm()
    End Sub

    Private Sub txt_deliveryto_GotFocus(sender As Object, e As EventArgs) Handles txt_location.GotFocus
        txt_location.BackColor = Color.Yellow
    End Sub

    Private Sub txt_deliveryto_KeyDown(sender As Object, e As KeyEventArgs) Handles txt_location.KeyDown
        If e.KeyCode = Keys.Enter Then
            Locationfindfrm()
            GridView1.Focus()
        End If
    ...
    Private Sub txt_deliveryto_LostFocus(sender As Object, e As EventArgs) Handles txt_location.LostFocus
