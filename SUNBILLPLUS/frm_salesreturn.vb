...
Public Class frm_salesreturn
    ...
    Dim SalesReturnpricelock As Boolean
    Dim ds_settings As New DataSet
    Dim GridviewEnter As Boolean
    ...
    Private Sub Execute()
        Try
            ...
            Call gridvisible(GridView1, False, "C_Itemid", "c_uomid", "c_decimal", "c_offeraddqty", "c_Offerlessqty", "c_addrs", "c_lessrs", "c_Rate")
    ...
    Private Sub dsopen()
        Try
            ...
            Sqlstr = "select isnull(numericvalue,0) as numericvalue from settings where process ='SALES RETURN PRICE LOCK'"
            cmd = New SqlCommand(Sqlstr, conn)
            cmd.CommandType = CommandType.Text
            da = New SqlDataAdapter(cmd)
            ds_settings = New DataSet
            ...
            If ds_settings.Tables(0).Rows.Count > 0 Then
                If ds_settings.Tables(0).Rows(0).Item("numericvalue") = 0 Then
                    SalesReturnpricelock = False
                Else
                    SalesReturnpricelock = True
                End If
            Else
                SalesReturnpricelock = False
            End If
    ...
    Private Sub Headercall(ByVal Headerid_v As Double)
        Try
            cmd = New SqlCommand("SELECT H.VCHNUM,H.HEADERID,H.VCHDATE,H.ITEMTYPE,H.NARRATION,ISNULL(H.PARTYID,0) AS PARTYID,case when h.partyid=0 or h.partyid is null then H.CUSTOMER else p.PTYNAME end as Customer,H.REFRENCE,isnull(cm.CARDNO,'') as CARDNO,isnull(CM.NAME,'') as NAME,isnull(H.CARDID,0) as CARDID,ISNULL(H.LOCATIONID,0) AS LOCATIONID,ISNULL(GM.GODOWNNAME,'') AS LOCATION FROM SALESRETURN_HEADER H " _
    ...
    Private Sub storechars(Optional ByVal pos As Integer = 0)
        Try
            ...
            cmd.Connection = conn
            cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = "GET_SALESRETURN_DETAIL"
            ...
            cmd.Connection = conn
            cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = "GET_SALESRETURNADDLESS"
            cmd.Parameters.Add("@Headerid", SqlDbType.Float).Value = Headerid_t
            da_detl = New SqlDataAdapter(cmd)
            ds_detl = New DataSet
            da_detl.Fill(ds_detl)
    ...
    Private Sub Gridfindfom(ByVal activerow As Integer, ByVal activecol As Integer, ByVal Dgv As DataGridView)
        Try
            ...
            Dim Costrate_t As Double, Rate_t As Double, OfferAddQty_t As Double, OfferlessQty_t As Double, Addqty_t As Double, LessQty_t As Double
            ...
            Select Case colindex_t
                ...
                Case fields1.c1_itemcode
                    ...
                    If Val_t = 0 Then
                        Sqlstr = "SELECT IM.ITEMCODE,IM.ITEMID,IM.ITEMDES as ITEMTAMILDES,IM.ITEMTAMILDES as ITEMDES,isnull(im.SELPRICERETAIL,0) as SELPRICERETAIL, ISNULL(IM.COSTPRICE,0) AS COSTRATE,UM.UOM,isnull(um.noofdecimal,0) as noofdecimal " _
                            & " ,ISNULL(IM.UOMID,0) AS UOMID,ISNULL(IM.OFFERADDQTY,0) AS OFFERADDQTY,ISNULL(IM.OFFERLESSQTY,0) AS OFFERLESSQTY,ISNULL(IM.ADDQTY,0) AS ADDQTY,ISNULL(IM.LESSQTY,0) AS LESSQTY FROM ITEM_MASTER IM " _
                        & " LEFT JOIN UOM_MASTER UM ON UM.MASTERID = IM.UOMID  where im.itemid not in (" & Nextcond_t & ") AND IM.INACTIVE = 1 ORDER BY IM.ITEMCODE "
                    Else
                        Sqlstr = "SELECT IM.ITEMCODE,IM.ITEMID,IM.ITEMDES,IM.ITEMTAMILDES,isnull(im.SELPRICERETAIL,0) as SELPRICERETAIL,ISNULL(IM.COSTPRICE,0) AS COSTRATE,UM.TAMILUOM AS UOM,isnull(um.noofdecimal,0) as noofdecimal " _
                            & " ,ISNULL(IM.UOMID,0) AS UOMID,ISNULL(IM.OFFERADDQTY,0) AS OFFERADDQTY,ISNULL(IM.OFFERLESSQTY,0) AS OFFERLESSQTY,ISNULL(IM.ADDQTY,0) AS ADDQTY,ISNULL(IM.LESSQTY,0) AS LESSQTY FROM ITEM_MASTER IM " _
                        & " LEFT JOIN UOM_MASTER UM ON UM.MASTERID = IM.UOMID  where im.itemid not in (" & Nextcond_t & ") AND IM.INACTIVE = 1 ORDER BY IM.ITEMCODE "
                    End If
                    ...
                    If ds_item.Tables(0).Rows.Count > 0 Then
                        ...
                        Else
                            ...
                            If Val_t = 0 Then
                                Sqlstr = "SELECT IM.ITEMCODE,IM.ITEMID,IM.ITEMDES as ITEMTAMILDES,IM.ITEMTAMILDES as ITEMDES,isnull(im.SELPRICERETAIL,0) as SELPRICERETAIL,ISNULL(IM.COSTPRICE,0) AS COSTRATE,UM.UOM " _
                                    & " ,IM.UOMID,ISNULL(IM.TAXPERC,0) AS TAXPERC,isnull(um.NOOFDECIMAL,0) as NOOFDECIMAL,ISNULL(IM.OFFERADDQTY,0) AS OFFERADDQTY,ISNULL(IM.OFFERLESSQTY,0) AS OFFERLESSQTY " _
                                    & " ,ISNULL(IM.ADDQTY,0) AS ADDQTY,ISNULL(IM.LESSQTY,0) AS LESSQTY FROM ITEM_MASTER IM " _
                      & " LEFT JOIN UOM_MASTER UM ON UM.MASTERID = IM.UOMID  WHERE IM.ITEMID NOT IN (" & Nextcond_t & ")  AND IM.INACTIVE = 1 and im.itemcode ='" & tmppassstr & "' ORDER BY IM.ITEMCODE "
                            Else
                                Sqlstr = "SELECT IM.ITEMCODE,IM.ITEMID,IM.ITEMDES,IM.ITEMTAMILDES,isnull(im.SELPRICERETAIL,0) as SELPRICERETAIL ,ISNULL(IM.COSTPRICE,0) AS COSTRATE " _
                                    & " ,UM.TAMILUOM AS UOM, IM.UOMID,ISNULL(IM.TAXPERC,0) AS TAXPERC,isnull(um.NOOFDECIMAL,0) as NOOFDECIMAL,ISNULL(IM.OFFERADDQTY,0) AS OFFERADDQTY, " _
                                    & " ISNULL(IM.OFFERLESSQTY,0) AS OFFERLESSQTY,ISNULL(IM.ADDQTY,0) AS ADDQTY,ISNULL(IM.LESSQTY,0) AS LESSQTY FROM ITEM_MASTER IM " _
                      & " LEFT JOIN UOM_MASTER UM ON UM.MASTERID = IM.UOMID  WHERE IM.ITEMID NOT IN (" & Nextcond_t & ")  AND IM.INACTIVE = 1 and im.itemcode ='" & tmppassstr & "' ORDER BY IM.ITEMCODE "
                            End If
    ...
    Private Sub Saveproc(ByVal editflag_t As Boolean)
        Try
            ...
            End If
            SavechhkFlg = False
            Headerid_t = GensaveSalesHead(IIf(editflag, 1, 0), Headerid_t, txt_vchnum.Text, DTP_Vchdate.Value, Cbo_itemtype.SelectedItem, txt_party.Text, _
                                              txt_reference.Text, Txt_custno.Text, Locationid_t, Gencompid, txt_narration.Text, Val(txt_netamt.Text), Partyid_t, Cardid_t)
            ...
            For i = 0 To GridView1.Rows.Count - 1
                ...
                If Itemid_t <> 0 And Qty_t <> 0 And amount_t <> 0 Then
                    ...
                    GensaveSalesdetl(Headerid_t, txt_vchnum.Text, DTP_Vchdate.Value, i + 1, Rate_t, amount_t, Gencompid, Itemid_t, Qty_t, remarks_v, CostRate_t)
                    SavechhkFlg = True
                End If
            Next
            ...
            For j = 0 To GridView2.Rows.Count - 1
                ...
                If Adtype_t <> "" And Addescid_t <> 0 And Adamt_t <> 0 Then
                    GensaveSALESaddlessdetl(Headerid_t, j + 1, Addescid_t, Adperc_t, Adamt_t, Adtype_t, Gencompid)
                    SavechhkFlg = True
                End If
            Next
            ...
            If Headerid_t <> 0 Then
                cmd = Nothing
                cmd = New SqlCommand("SALESRETURN_AC_SALE_UPD", conn)
                cmd.CommandType = CommandType.StoredProcedure
        ...
        Try
            ...
            If Char.IsDigit(e.KeyChar) Then
                If Text.IndexOf(".") <> -1 Then
                    If Not GridView2.CurrentCell Is Nothing Then
                        If GridView2.CurrentCell.ColumnIndex = fields2.c2_Perc Or GridView2.CurrentCell.ColumnIndex = fields2.c2_Amount Then
            ...
            If Not GridView2.CurrentCell Is Nothing Then
                If GridView2.CurrentCell.ColumnIndex = fields2.c2_Desc Or GridView2.CurrentCell.ColumnIndex = fields2.c2_Type Then
                    e.Handled = False
                End If
            End If
    ...
    Private Sub Textbox_TextChanged(ByVal sender As Object, ByVal e As System.EventArgs)
        Try
            ...
            '        'GridView1.Rows(GridView1.CurrentCell.RowIndex).Cells(fields1.c1_amount ) =         
            '    End If
            'End If
            '        If Not CType(sender, TextBox).Text.Trim.Length = 0 Then
    ...
    Private Sub GridView1_CellEndEdit(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GridView1.CellEndEdit
        Try
            If e.RowIndex >= 0 And e.ColumnIndex >= 0 Then
                ...
                GridView1.Rows(e.RowIndex).DefaultCellStyle.Font = font1
                ...
                If GridView1.Rows.Count - 1 = e.RowIndex Then
                    If IsValidRow(GridView1, "C_Itemcode", "C_Beforeexcessqty") Then
                        GridView1.Rows.Add(1)
                    End If
                End If
    ...
    Private Sub GridView1_CellValueChanged(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GridView1.CellValueChanged
        Try
            ...
            If (e.ColumnIndex = fields1.c1_rate Or e.ColumnIndex = fields1.c1_qty) And e.RowIndex >= 0 Then
                tmpqty_t = IIf(IsDBNull(GridView1.Item(fields1.c1_qty, e.RowIndex).Value), 0, GridView1.Item(fields1.c1_qty, e.RowIndex).Value)
    ...
    Public Sub Print_Rpt(ByVal Headerid_v As Double)
        Try
            ...
            rm.Init(conn, "SALES", Servername_t, Headerid_v, Nothing, Nothing, "", Databasename_t, 0, False, , , , , , SystemName_t)
    ...
    Private Sub Partyfindfrm()
        Try
            ...
            If cnt > 0 Then
                ...
                fm.Frm_Left = 880
                fm.Frm_Top = 101
                fm.MainForm = New frm_salesreturn
    ...
    System.ComponentModel.TypeDescriptor.GetConverter(GetType(Font))
     ...
     CType(converter.ConvertFromString(Font_m & " ," & Size_m), Font)
            ...
            If SalesReturnpricelock = True Then
                gridreadonly(GridView1, True, "c_selrate")
                gridreadonly_Color(GridView1, Readonlycolor_t, "c_selrate")
            End If
