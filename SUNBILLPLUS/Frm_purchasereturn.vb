...
Public Class Frm_purchasereturn
    ...
    Private Sub Execute()
        Try
            ...
            Call gridreadonly(GridView1, True, "C_Itemdes", "c_uom", "c_uomid", "C_Amount", "c_costrate")
            Call gridreadonly_Color(GridView1, Readonlycolor_t, "C_Itemdes", "c_uom", "c_uomid", "C_Amount", "c_costrate")
            Call gridvisible(GridView1, False, "C_Itemid", "c_uomid", "c_decimal", "c_Rate")
            ...
            ds_settings = Nothing
            ' ds_settings.Clear()
            Sqlstr = "SELECT ISNULL(NUMERICVALUE,0 ) AS NUMERICVALUE FROM SETTINGS WHERE PROCESS  ='PURCHASE RETURN RATE LOCK' "
            cmd = New SqlCommand(Sqlstr, conn)
            cmd.CommandType = CommandType.Text
            da_settings = New SqlDataAdapter(cmd)
            ds_settings = New DataSet
            ...
            If costRatelock = True Then
                Call gridreadonly(GridView1, costRatelock, "c_costrate")
    ...
    Private Sub Headercall(ByVal Headerid_v As Double)
        Try
            cmd = New SqlCommand("SELECT H.VCHNUM,H.HEADERID,H.VCHDATE,isnull(H.reference,'') as reference,isnull(h.vehicleno,'') as vehicleno,isnull(h.pdcno,'') as pdcno " _
                                 & " ,h.pdcdate,isnull(H.NARRATION,'') as NARRATION,ISNULL(H.PARTYID,0) AS PARTYID, p.PTYNAME as Customer,ISNULL(H.LOCATIONID,0) AS LOCATIONID,ISNULL(GM.GODOWNNAME,'') AS LOCATION FROM PURCHASERETURN_HEADER H " _
                                 & "LEFT JOIN GODOWN_MASTER GM ON GM.MASTERID = H.LOCATIONID LEFT JOIN PARTY P ON P.PTYCODE = H.PARTYID WHERE H.HEADERID = " & Headerid_v & " ", conn)
            cmd.CommandType = CommandType.Text
    ...
    Private Sub storechars(Optional ByVal pos As Integer = 0)
        Try
         ...
         '   txt_pdcno.Text = ds_head.Tables(0).Rows(rowid_t).Item("pdcno").ToString
            ...
            cmd.Connection = conn
            cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = "GET_PURCHASERETURN_DETAIL"
            ...
            cmd.Connection = conn
            cmd.CommandType = CommandType.StoredProcedure
            cmd.CommandText = "GET_PURCHASERETURNADDLESS"
            cmd.Parameters.Add("@Headerid", SqlDbType.Float).Value = Headerid_t
            da_detl = New SqlDataAdapter(cmd)
            ds_detl = New DataSet
            da_detl.Fill(ds_detl)
...
End Sub
            ...
            Select Case colindex_t
                Case fields1.c1_itemcode
                    ...
                    If ds_item.Tables(0).Rows.Count > 0 Then
                      ...
                      & " LEFT JOIN UOM_MASTER UM ON UM.MASTERID = IM.UOMID  WHERE IM.ITEMID NOT IN (" & Nextcond_t & ")  and im.itemcode ='" & tmppassstr & "' AND IM.INACTIVE = 1  ORDER BY IM.ITEMCODE "
                            Else
    ...
    Private Sub Saveproc(ByVal editflag_t As Boolean)
        Try
            ...
            End If
            SavechhkFlg = False
            Headerid_t = GensavePurchaseReturnHead(IIf(editflag, 1, 0), Headerid_t, txt_vchnum.Text, DTP_Vchdate.Value, Partyid_t, txt_reference.Text, Nothing, "", Locationid_t,
            ...
            For i = 0 To GridView1.Rows.Count - 1
                ...
                If Itemid_t <> 0 And Qty_t <> 0 And amount_t <> 0 Then
                    ...
                    GensavePurchaseReturndetl(Headerid_t, txt_vchnum.Text, DTP_Vchdate.Value, i + 1, Rate_t, amount_t, Gencompid, Itemid_t, Qty_t, remarks_v, CostRate_t)
                    SavechhkFlg = True
                End If
            Next
            ...
            For j = 0 To GridView2.Rows.Count - 1
                ...
                If Adtype_t <> "" And Addescid_t <> 0 And Adamt_t <> 0 Then
                    GensavePURCHASEREtaddlessdetl(Headerid_t, j + 1, Addescid_t, Adperc_t, Adamt_t, Adtype_t, Gencompid)
                    SavechhkFlg = True
                End If
            Next
            ...
            If Headerid_t <> 0 Then
                If Acpostingwithlrno_t = True Then
                    cmd = Nothing
                    cmd = New SqlCommand("AC_PURRET_UPD", conn)
                    cmd.CommandType = CommandType.StoredProcedure
        ...
        Try
            ...
            If Char.IsDigit(e.KeyChar) Then
                If Text.IndexOf(".") <> -1 Then
                    If GridView1.CurrentCell.ColumnIndex = fields1.c1_rate Or GridView1.CurrentCell.ColumnIndex = fields1.c1_costrate Then
    ...
    Public Sub Deleteproc(ByVal Headerid_v As Double)
        ...
        Try
            If MsgBox("Are you sure ?", MsgBoxStyle.YesNo + MsgBoxStyle.DefaultButton2, "Delete") = MsgBoxResult.Yes Then
                ...
                Call GendelPurchaseReturn(Headerid_v)
    ...
    Public Sub Print_Rpt(ByVal Headerid_v As Double)
        Try
            ...
            rm.Init(conn, "PURCHASE RETURN", Servername_t, Headerid_v, Nothing, Nothing, "", Databasename_t, 0, False)
    ...
    Private Sub Buttonsettings(ByVal visflag_t As Boolean, ByVal cap_t As String)
        Try
            ...
            Call gridreadonly(GridView1, Not visflag_t, "C_Itemcode", "C_Itemdes", "c_Rate", "C_Amount", "c_uom", "c_remarks")
        Catch ex As Exception
            MessageBox.Show(ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
 ...
 Private Sub GridView1_CellContentClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles GridView1.CellContentClick
    ...
    Private Sub GridView1_EditingControlShowing(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewEditingControlShowingEventArgs) Handles GridView1.EditingControlShowing
        ...
        Select Case GridView1.CurrentCell.ColumnIndex
            Case fields1.c1_qty, fields1.c1_rate, fields1.c1_amount, fields1.c1_itemcode, fields1.c1_costrate
                AddHandler CType(e.Control, TextBox).KeyPress, AddressOf TextBox_keyPress
    ...
    'Private Sub txt_reference_GotFocus(sender As Object, e As EventArgs)
    '    txt_pdcno.BackColor = Color.Yellow
    'End Sub

    'Private Sub txt_reference_KeyDown(sender As Object, e As KeyEventArgs)
    '    If e.KeyCode = Keys.Enter Then
    '        Dtp_pdcdate.Focus()
    '    End If
    'End Sub

    'Private Sub txt_reference_LostFocus(sender As Object, e As EventArgs)
    ...
    Private Sub Dtp_pdcdate_KeyDown(sender As Object, e As KeyEventArgs)
        If e.KeyCode = Keys.Enter Then
            Txt_vehicleno.Focus()
        End If
    End Sub
