...
Public Class frm_order
    ...
    Dim Headerid_t As Double, AcDescid_t As Double
    ...
    Private Sub Headercall(ByVal Headerid_v As Double)
        Try
            cmd = New SqlCommand("SELECT H.VCHNUM,H.HEADERID,H.VCHDATE,ISNULL(H.PARTYID,0) AS PARTYID,P.PTYNAME AS CUSTOMER,H.NARRATION  FROM ORDER_HEADER H " _
    ...
    Private Sub storechars(Optional ByVal pos As Integer = 0)
        Try
            ...
            cmd.CommandText = "GET_ORDERDETAIL"
    ...
    Private Sub Gridfindfom(ByVal activerow As Integer, ByVal activecol As Integer, ByVal Dgv As DataGridView)
        Try
            ...
            Dim uomid_v As Double, Rate_t As Double
                ...
                Case fields1.c1_itemcode
                    ...
                    If ds_item.Tables(0).Rows.Count > 0 Then
                        ...
                        Else
                            ...
                            If Val_t = 0 Then
                                Sqlstr = "SELECT IM.ITEMCODE,IM.ITEMID,IM.ITEMDES AS ITEMTAMILDES,isnull(im.SELPRICERETAIL,0) as SELPRICERETAIL,IM.ITEMTAMILDES AS ITEMDES,ISNULL(IM.COSTPRICE,0) AS COSTRATE,UM.UOM,IM.UOMID,ISNULL(IM.TAXPERC,0) AS TAXPERC,isnull(um.NOOFDECIMAL,0) as NOOFDECIMAL FROM ITEM_MASTER IM " _
                      ...
                      & " LEFT JOIN UOM_MASTER UM ON UM.MASTERID = IM.UOMID  WHERE IM.ITEMID NOT IN (" & Nextcond_t & ")  and im.itemcode ='" & tmppassstr & "' ORDER BY IM.ITEMCODE "
                        ...
                        If ds_item.Tables(0).Rows.Count > 0 And fm.VarNewid <> 0 Then
                            ...
                            For i = 0 To ds_item.Tables(0).DefaultView.Count - 1
                                ...
                                uomid_v = ds_item.Tables(0).Rows(i + index_t).Item("uomid")
                                ...
                                If Tmpitemid_t <> 0 Then
                                    ...
                                    GridView1.Rows(activerow + i).Cells(fields1.c1_uomid).Value = uomid_v
    ...
    Private Sub Saveproc(ByVal editflag_t As Boolean)
        Try
            ...
            Dim Adtype_t As String, Adamt_t As Double, Adperc_t As Double, Addescid_t As Double, CostRate_t As Double, uomid_t As Double
            ...
            End If
            SavechhkFlg = False
            Headerid_t = GensaveOrderHead(IIf(editflag, 1, 0), Headerid_t, txt_vchnum.Text, DTP_Vchdate.Value, txt_party.Text, _
                                              txt_reference.Text, Gencompid, txt_narration.Text, Val(txt_totamount.Text), Partyid_t)
            ...
            For i = 0 To GridView1.Rows.Count - 1
                ...
                If Itemid_t <> 0 And Qty_t <> 0 Then
                    ...
                    GensaveOrderdetl(Headerid_t, txt_vchnum.Text, DTP_Vchdate.Value, i + 1, Rate_t, amount_t, Gencompid, Itemid_t, Qty_t, remarks_v, uomid_t)
                    SavechhkFlg = True
                End If
            Next
    ...
    Public Sub Print_Rpt(ByVal Headerid_v As Double)
        Try
            ...
            rm.Init(conn, "PURCHASE ORDER", Servername_t, Headerid_v, Nothing, Nothing, "", Databasename_t, 0, False, Nothing)
